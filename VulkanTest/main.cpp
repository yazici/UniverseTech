...
private:
	...
	void setupInput() {
		...
		auto kbfunc = [](GLFWwindow * w, int key, int scancode, int action, int mods) {
			static_cast<VulkanWindowApp*>(glfwGetWindowUserPointer(w))->handleKeyboardInput(w, key, scancode, action, mods);
		};

		glfwSetKeyCallback(window, kbfunc);

		auto mpfunc = [](GLFWwindow * w, double xpos, double ypos) {
			static_cast<VulkanWindowApp*>(glfwGetWindowUserPointer(w))->handleMouseInput(w, xpos, ypos);
		};

		glfwSetCursorPosCallback(window, mpfunc);
