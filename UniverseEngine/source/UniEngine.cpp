...
void UniEngine::setupDescriptorSetLayout() {
	...
	VK_CHECK_RESULT(vkCreatePipelineLayout(device, &pPipelineLayoutCreateInfo, nullptr, &pipelineLayouts.forward));
...
void UniEngine::preparePipelines() {
	...
	shaderStages[0] = loadShader(getAssetPath() + "shaders/omnishader.vert.spv", VK_SHADER_STAGE_VERTEX_BIT);
	shaderStages[1] = loadShader(getAssetPath() + "shaders/omnishader.frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT);


	VkGraphicsPipelineCreateInfo pipelineCreateInfo =
		vks::initializers::pipelineCreateInfo(
			pipelineLayouts.forward,
			renderPass,
			0);
	...
	VK_CHECK_RESULT(vkCreateGraphicsPipelines(device, pipelineCache, 1, &pipelineCreateInfo, nullptr, &pipelines.forward));
...
void UniEngine::buildCommandBuffers() {
	...
	for(int32_t i = 0; i < drawCmdBuffers.size(); ++i) {
		...
		uint32_t dummy = 0;
		vkCmdBindDescriptorSets(drawCmdBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayouts.forward, 0, 1, &m_descriptorSet, 0, nullptr);
		...
		vkCmdBindPipeline(drawCmdBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, pipelines.forward);
		vkCmdDraw(drawCmdBuffers[i], 3, 1, 0, 0);
		...
		auto models = m_CurrentScene->GetModels();
		for_each(models.begin(), models.end(), [this, &index, dynamicAlignment, i](std::shared_ptr<UniModel> model) {
			VkDeviceSize offsets[1] = { 0 };
			uint32_t dynamicOffset = index * static_cast<uint32_t>(dynamicAlignment);
			vkCmdBindDescriptorSets(drawCmdBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayouts.forward, 0, 1, &model->m_DescriptorSet, 1, &dynamicOffset);
			vkCmdBindVertexBuffers(drawCmdBuffers[i], VERTEX_BUFFER_BIND_ID, 1, &model->m_Model.vertices.buffer, offsets);
			vkCmdBindIndexBuffer(drawCmdBuffers[i], model->m_Model.indices.buffer, 0, VK_INDEX_TYPE_UINT32);
			vkCmdDrawIndexed(drawCmdBuffers[i], model->m_Model.indexCount, 1, 0, 0, 0);
			index++;
		});
