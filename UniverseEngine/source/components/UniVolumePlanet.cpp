...
UniVolumePlanet::UniVolumePlanet(double radius /*= 1.0*/, double maxHeightOffset /*= 0.1*/, double maxDepthOffset /*= 0.1*/, uint16_t divisions /*= 10*/, bool hasOcean /*= false*/){
...
void UniVolumePlanet::Initialize() {

	std::vector<Tet> tets;
	double u = pow(2.0, m_Divisions);
	
	tets.emplace_back(glm::dvec3(0, 0, 0), glm::dvec3(u, u, u), glm::dvec3(u, u, 0), glm::dvec3(u, 0, 0));
	tets.emplace_back(glm::dvec3(0, 0, 0), glm::dvec3(u, u, u), glm::dvec3(u, u, 0), glm::dvec3(0, u, 0));
	tets.emplace_back(glm::dvec3(0, 0, 0), glm::dvec3(u, u, u), glm::dvec3(0, u, u), glm::dvec3(0, 0, u));
	tets.emplace_back(glm::dvec3(0, 0, 0), glm::dvec3(u, u, u), glm::dvec3(0, u, u), glm::dvec3(0, u, 0));
	tets.emplace_back(glm::dvec3(0, 0, 0), glm::dvec3(u, u, u), glm::dvec3(u, 0, u), glm::dvec3(u, 0, 0));	
	tets.emplace_back(glm::dvec3(0, 0, 0), glm::dvec3(u, u, u), glm::dvec3(u, 0, u), glm::dvec3(0, 0, u));
	
	for(auto& tet : tets) {

		tet.CalculateLongestEdge();

		auto le = tet.le;
		auto d = tet.d;

		auto dir = le.second - le.first;
		dir /= u;

		auto u2 = u / 2.0;

		if(diamond_map.find(d->pos) == diamond_map.end()) {
			diamond_map[d->pos] = d;
			d->type = static_cast<uint32_t>(std::distance(diamond_types.begin(), diamond_types.insert(dir).first));
		}

		std::cout << "Made a tet with diamond centre: " << d->pos.x << ", " << d->pos.y << ", " << d->pos.z << std::endl;
		std::cout << "Longest edge direction: " << dir.x << ", " << dir.y << ", " << dir.z << std::endl;

		if(offset_map.find(d->type) == offset_map.end()) {
			auto od = OffsetLookup();
			offset_map[d->type] = od;
		}
		Tet tt = Tet((d->pos - tet.v0) / u2, (d->pos - tet.v1) / u2, (d->pos - tet.v2) / u2, (d->pos - tet.v3) / u2);
		offset_map[d->type].tets.insert(tt);

	}

	for(auto& kv : offset_map) {
		auto tCount = 0;
		for(auto& t : kv.second.tets) {
			for(auto& v : { t.v0, t.v1, t.v2, t.v3 }) {
				std::cout << "Found diamond type " << kv.first << ", tet " << tCount << ": " << v.x << ", " << v.y << ", " << v.z << std::endl;
			}
			tCount++;
		}
...
void UniVolumePlanet::CreateGrid() {

	m_GridPoints.clear();
	float increment = 2.0f / m_Divisions;
	...
	assert(m_GridPoints.size() == (m_Divisions + 1) * (m_Divisions + 1));
}

void UniVolumePlanet::CreateOceanTriangles() {
	m_OceanIndices.clear();
	for(int y = 0; y < m_Divisions; y++) {
		for(int x = 0; x < m_Divisions; x++) {
			m_OceanIndices.push_back(y * (m_Divisions + 1) + x);
			m_OceanIndices.push_back((y + 1) * (m_Divisions + 1) + x);
			m_OceanIndices.push_back(y * (m_Divisions + 1) + x + 1);

			m_OceanIndices.push_back(y * (m_Divisions + 1) + x + 1);
			m_OceanIndices.push_back((y + 1) * (m_Divisions + 1) + x);
			m_OceanIndices.push_back((y + 1) * (m_Divisions + 1) + x + 1);

		}
	}

	assert(m_OceanIndices.size() == m_Divisions * m_Divisions * 6);
}

void UniVolumePlanet::CreateQuads() {
	m_Indices.clear();
	for(int y = 0; y < m_Divisions; y++) {
		for(int x = 0; x < m_Divisions; x++) {
			m_Indices.push_back(y * (m_Divisions + 1) + x);
			m_Indices.push_back((y + 1) * (m_Divisions + 1) + x);
			m_Indices.push_back((y + 1) * (m_Divisions + 1) + x + 1);
			m_Indices.push_back(y * (m_Divisions + 1) + x + 1);
			
		}
	}

	assert(m_Indices.size() == m_Divisions * m_Divisions * 4);
}

glm::vec3 UniVolumePlanet::RotatePointToCamera(glm::vec3 point)
{
	glm::quat rot = glm::rotation(glm::vec3(0.f, 0.f, -1.f), m_CurrentCameraPos);
	return rot * point;
}

double UniVolumePlanet::CalculateZOffset() {
...
void UniVolumePlanet::SetCameraPosition(glm::vec3& cam) {
	m_CurrentCameraPos = cam;
	//std::cout << "Camera relative to planet: " << cam.x << ", " << cam.y << ", " << cam.z << std::endl;
}

double UniVolumePlanet::GetPositionOffset(glm::vec3& pos) {
	return glm::length(pos);
}

void UniVolumePlanet::UpdateMesh() {
	m_MeshVerts.clear();
	m_OceanVerts.clear();
...
void UniVolumePlanet::UpdateBuffers() {
	...
	vks::Buffer vertexStaging, indexStaging, oceanVertexStaging, oceanIndexStaging;
	...
	if(m_HasOcean) {
		...
		uint32_t oceanIndexBufferSize = static_cast<uint32_t>(m_OceanIndices.size() * sizeof(uint32_t));
		// Index buffer
		VK_CHECK_RESULT(device->createBuffer(
			VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
			VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
		...
		vkCmdCopyBuffer(copyCmd, oceanIndexStaging.buffer, m_OceanIndexBuffer.buffer, 1, &copyRegion);
...
void UniVolumePlanet::UpdateUniformBuffers(glm::mat4& modelMat) {
	auto& engine = UniEngine::GetInstance();
	...
	vks::Frustum frustum;
	frustum.update(m_UniformBufferData.projMat * m_UniformBufferData.viewMat * m_UniformBufferData.modelMat);
	memcpy(m_UniformBufferData.frustumPlanes, frustum.planes.data(), sizeof(glm::vec4) * 6);
...
float UniVolumePlanet::GetAltitude(glm::vec3& point) {
...
void UniVolumePlanet::SetZOffset(float  value) {
	m_ZOffset = value;
}

void UniVolumePlanet::CreateBuffers() {

	uint32_t vertexBufferSize = static_cast<uint32_t>(m_MeshVerts.size() * sizeof(glm::vec3));
	uint32_t oceanVertexBufferSize = static_cast<uint32_t>(m_OceanVerts.size() * sizeof(glm::vec3));
	uint32_t indexBufferSize = static_cast<uint32_t>(m_Indices.size() * sizeof(uint32_t));
	uint32_t oceanIndexBufferSize = static_cast<uint32_t>(m_OceanIndices.size() * sizeof(uint32_t));
	uint32_t ubSize = static_cast<uint32_t>(sizeof(UniVolumePlanet::UniformBufferData));
	...
	m_Material->SetBuffer("oceanindex", std::make_shared<vks::Buffer>(m_OceanIndexBuffer));
	if(m_HasOcean) {
		m_Material->SetBuffer("oceanvertex", std::make_shared<vks::Buffer>(m_OceanVertexBuffer));
	}
}

void UniVolumePlanet::DestroyBuffers() {
...
void UniVolumePlanet::MakeContintentTexture() {
...
void UniVolumePlanet::UpdateStorageBuffer() {
	auto& engine = UniEngine::GetInstance();
...
void UniVolumePlanet::UpdateTime(float t) {
	m_Material->SetTime(t);
}

double UniVolumePlanet::GetRadius() {
	return m_Radius;
}

uint32_t UniVolumePlanet::AddNoiseLayer(NoiseType type, uint32_t octaves, float seed/*=1337.0f*/) {
...
void UniVolumePlanet::MakeRampTexture() {
	auto& engine = UniEngine::GetInstance();
