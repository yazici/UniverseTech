...
// iOS & macOS: getAssetPath() implemented externally to allow access to
...
class Texture2D : public Texture {
 public:
   ...
   * @param (Optional) forceLinear Force linear tiling (not advised, defaults to
   * false)
   *
   */
  void loadFromFile(
    ...
    if (useStaging) {
      ...
      bufferCreateInfo.size = bitmap.stride * bitmap.height;
      ...
      memcpy(data, bitmap.address<mango::u8>(0, 0), bitmap.height * bitmap.stride);
      ...
      for (uint32_t i = 0; i < mipLevels; i++) {
        ...
        offset += static_cast<uint32_t>(bitmap.height * bitmap.stride);
      }
