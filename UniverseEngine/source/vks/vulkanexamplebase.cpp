...
void VulkanExampleBase::submitFrame() {
  ...
  VK_CHECK_RESULT(swapChain.queuePresent(
      queue, currentBuffer,
...
bool VulkanExampleBase::initVulkan() {
  ...
  // Create a semaphore used to synchronize command submission
  // Ensures that the image is not presented until all commands for the UI
  // overlay have been sumbitted and executed Will be inserted after the render
  // complete semaphore if the UI overlay is enabled
  VK_CHECK_RESULT(vkCreateSemaphore(device, &semaphoreCreateInfo, nullptr,
            ...
            case AMOTION_EVENT_ACTION_UP: {
              ...
              if (eventTime - downTime <= vks::android::TAP_TIMEOUT) {
                float deadZone = (160.f / vks::android::screenDensity) *
                                 ...
                                 vks::android::TAP_SLOP;
                float x =
                    AMotionEvent_getX(event, 0) - vulkanExample->touchPos.x;
                float y =
                    AMotionEvent_getY(event, 0) - vulkanExample->touchPos.y;
                if ((x * x + y * y) < deadZone) {
