...
void PlayerControlSystem::receive(ECS::World* world, const InputEvent& event) {
	...
	case Input::AxisYaw:
		...
		break;
	case Input::ButtonRightClick:
		isFullStop = event.value == 1.f;
		break;
	case Input::ButtonBoostUp:
		if(m_BoostFactor < 1024.f * 8.f)
			m_BoostFactor *= 2.0f;
		break;
	case Input::ButtonBoostDown:
		if(m_BoostFactor > 1.f / 1024.f * 8.f)
			m_BoostFactor /= 2.0f;
		break;
	case Input::ButtonRollLeft:
		m_InputRotation.z = event.value;
		break;
	case Input::ButtonRollRight:
		m_InputRotation.z = -event.value;
		break;
    case Input::ButtonExperiment:
        UniEngine::GetInstance()->GetSceneManager()->CycleScenes();
        break;
    }
}
